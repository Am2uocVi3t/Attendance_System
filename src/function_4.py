...
def train_face_recognizer(data_dir, log_func=None):
    # Giảm nhiễu với các tham số trong model LBPHFaec
    ...
    subfolders = ["images", "images_with_glasses"]
    ...
    if len(faces) < 10:
        if log_func:
            log_func("[WARN] Not enough training data. At least 10 face images from at least 1 person required.")
        return None, None
